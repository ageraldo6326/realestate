<section class="advisor-client-report" aria-labelledby="advisor-client-report-title">
    <header class="advisor-client-report__header">
        ...
        <?php if($this->hasDateRange && ! $this->invalidDateRange): ?>
            <div class="advisor-client-report__total" aria-live="polite"><span>Contactos encontrados</span><strong><?php echo e(number_format($clientes_por_asesores->sum('total'))); ?></strong></div>
        <?php endif; ?>
    </header>
    ...
    <form class="advisor-client-report__filters" wire:submit.prevent="applyFilters" aria-label="Filtrar clientes por período">
        ...
        <div class="advisor-client-report__filter-actions">
            ...
            <?php if($fecha_ini !== '' || $fecha_fin !== ''): ?><button type="button" wire:click="clearFilters" class="btn btn-outline-secondary" wire:loading.attr="disabled" wire:target="clearFilters">Limpiar</button><?php endif; ?>
        </div>
    ...
    <?php elseif($clientes_por_asesores->isEmpty()): ?>
    ...
    <?php else: ?>
        <div class="advisor-client-report__summary" aria-live="polite"><strong><?php echo e(number_format($clientes_por_asesores->count())); ?></strong> asesores con clientes captados entre <?php echo e(\Illuminate\Support\Carbon::parse($fecha_ini)->format('d/m/Y')); ?> y <?php echo e(\Illuminate\Support\Carbon::parse($fecha_fin)->format('d/m/Y')); ?>.</div>
        <div class="advisor-client-report__list" wire:loading.class="is-loading" wire:target="applyFilters, clearFilters">
            <?php $__currentLoopData = $clientes_por_asesores; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $clientes_por_asesor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <article class="advisor-client-card" wire:key="advisor-client-<?php echo e($clientes_por_asesor->userid ?: 'unassigned'); ?>">
                    <div class="advisor-client-card__identity"><span class="advisor-client-card__avatar" aria-hidden="true"><?php echo e(\Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($clientes_por_asesor->asesor_nombre ?: 'S', 0, 1))); ?></span><div><h2><?php echo e($clientes_por_asesor->asesor_nombre ?: 'Sin asesor registrado'); ?></h2><p><?php echo e($clientes_por_asesor->userid ? 'Asesor responsable' : 'Registro sin asesor asociado'); ?></p></div></div>
                    <div class="advisor-client-card__count"><span>Clientes captados</span><strong><?php echo e(number_format($clientes_por_asesor->total)); ?></strong></div>
                    <?php if($clientes_por_asesor->userid): ?>
                        <a href="<?php echo e(route('verclientesporasesor', ['asesorid' => $clientes_por_asesor->userid, 'fecha_ini' => $fecha_ini, 'fecha_fin' => $fecha_fin])); ?>" class="btn btn-outline-secondary advisor-client-card__action">Ver clientes</a>
                    <?php else: ?>
