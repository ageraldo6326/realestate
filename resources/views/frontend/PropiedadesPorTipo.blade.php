...
@section('seo_title', 'Propiedades de Tipo ' . ($tipo->tipo ?? '') . ' — ' . ($inmo->titulo ?? 'Portal Inmobiliario'))
@section('seo_description', 'Explora propiedades de tipo ' . ($tipo->tipo ?? '') . ' disponibles en venta y alquiler.')
...
<section class="page-hero" aria-label="Propiedades por Tipo">
    <div class="container">
        <h1 class="page-hero-title">{{ $tipo->tipo ?? 'Propiedades' }}</h1>
...
<section class="listings-section">
    <div class="container">
        @livewire('buscar-propiedades-por-tipos', ['tipo_id' => $tipo_id, 'tipo' => $tipo])
    </div>
</section>
