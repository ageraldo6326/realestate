...
@section('title', 'Clientes del asesor')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('consultaClientesAsesor') }}">Clientes por asesor</a></li>
    <li class="breadcrumb-item active">Detalle</li>
@endsection

@section('page_title', 'Clientes del asesor')

@section('content')
    <div class="container-fluid py-3 py-md-4 advisor-client-detail">
        <header class="advisor-client-detail__header">
            <div><a href="{{ route('consultaClientesAsesor') }}" class="advisor-client-detail__back"><i class="fas fa-arrow-left" aria-hidden="true"></i> Volver a la consulta</a><h1>Clientes del asesor</h1><p>Registros entre {{ \Illuminate\Support\Carbon::parse($fecha_ini)->format('d/m/Y') }} y {{ \Illuminate\Support\Carbon::parse($fecha_fin)->format('d/m/Y') }}.</p></div>
            <div class="advisor-client-detail__count"><span>Total de clientes</span><strong>{{ number_format($clientes->count()) }}</strong></div>
        </header>

        @forelse ($clientes as $cliente)
            <article class="advisor-client-detail__card">
                <div class="advisor-client-detail__identity"><span class="advisor-client-detail__avatar" aria-hidden="true">{{ \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($cliente->nombre ?: 'C', 0, 1)) }}</span><div><h2>{{ $cliente->nombre ?: 'Cliente sin nombre' }}</h2><p>{{ $cliente->created_at ? $cliente->created_at->format('d/m/Y') : 'Fecha no disponible' }}</p></div></div>
                <dl class="advisor-client-detail__metadata"><div><dt>Teléfono</dt><dd>{{ $cliente->telefono ?: 'No registrado' }}</dd></div><div><dt>Correo</dt><dd>{{ $cliente->email ?: 'No registrado' }}</dd></div><div><dt>Rango de interés</dt><dd>{{ $cliente->precio_mini || $cliente->precio_max ? number_format($cliente->precio_mini ?: 0) . ' – ' . number_format($cliente->precio_max ?: 0) : 'No registrado' }}</dd></div><div><dt>Contacto</dt><dd>{{ $cliente->contact_at ?: 'No registrado' }}</dd></div></dl>
                <a href="{{ route('vercliente', $cliente->id) }}" class="btn btn-outline-secondary advisor-client-detail__action">Ver ficha</a>
            </article>
        @empty
            <div class="advisor-client-detail__empty" role="status"><i class="fas fa-users" aria-hidden="true"></i><h2>No hay clientes para este asesor</h2><p>Prueba con otro período de consulta.</p><a href="{{ route('consultaClientesAsesor') }}" class="btn btn-outline-primary">Volver a la consulta</a></div>
        @endforelse
    </div>
