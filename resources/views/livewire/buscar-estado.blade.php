                    ...
                    <table class="table align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($estados as $estado)
                                <tr>
                                    <td class="font-weight-semibold">{{ $estado->id }}</td>
                                    <td>{{ $estado->estado }}</td>
                                    <td class="d-none d-md-table-cell text-muted small">{{ $estado->created_at }}</td>
                                    <td class="text-right">
                                        <button type="button" class="btn btn-outline-primary btn-sm mr-1"
                                            wire:click="edit({{ $estado->id }})" data-toggle="modal" data-target="#modalForm">
                                            Editar
                                        </button>
                                        <button type="button" class="btn btn-outline-danger btn-sm"
                                            wire:click="$emit('generarBorrarEstadoSweetAlert', {{ $estado->id }})">
                                            Borrar
                                        </button>
                                    </td>
                                </tr>
        ...
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header border-0 pb-0 px-4 pt-4">
                    <div>
                        <div class="text-uppercase small text-muted">{{ $Id ? 'Edicion' : 'Nuevo estado' }}</div>
                        <h4 class="modal-title mb-0">{{ $Id ? 'Editar estado' : 'Registrar estado' }}</h4>
                    </div>
