            ...
            <div class="card-body p-4">
                ...
                <div class="table-responsive">
                    <table class="table align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($testimonios as $testimonioItem)
                                <tr>
                                    <td class="font-weight-semibold">{{ $testimonioItem->id }}</td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            @if ($testimonioItem->cliente_foto)
                                                <img src="{{ asset('assets/' . $testimonioItem->cliente_foto) }}" class="content-avatar mr-3" alt="{{ $testimonioItem->cliente }}">
                                            @endif
                                            <span>{{ $testimonioItem->cliente }}</span>
                                        </div>
                                    </td>
                                    <td class="d-none d-md-table-cell text-muted">{!! \Illuminate\Support\Str::limit(strip_tags($testimonioItem->testimonio), 80) !!}</td>
                                    <td class="text-right">
                                        <button type="button" class="btn btn-outline-primary btn-sm mr-1" wire:click="edit({{ $testimonioItem->id }})" data-toggle="modal" data-target="#modalForm">Editar</button>
                                        <button type="button" class="btn btn-outline-danger btn-sm" wire:click="$emit('generarBorrarTestimonioSweetAlert', {{ $testimonioItem->id }})">Borrar</button>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                ...
                <div class="mt-4">{{ $testimonios->links() }}</div>
            </div>
        </section>
    </div>
        ...
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header border-0 pb-0 px-4 pt-4">
                    <div>
                        <div class="text-uppercase small text-muted">{{ $Id ? 'Edicion' : 'Nuevo testimonio' }}</div>
                        <h4 class="modal-title mb-0">{{ $Id ? 'Editar testimonio' : 'Registrar testimonio' }}</h4>
                    </div>
                            ...
                            <div class="card content-panel">
                                <div class="card-body p-4">
                                    ...
                                    <div class="form-group mb-0" wire:ignore>
                                        ...
                                        <textarea class="form-control" id="testimonio-editor" rows="10">{{ $testimonio }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-xl-5">
                            <div class="card content-panel">
                                <div class="card-body p-4" x-data="{ isUploading: false, progress: 0 }"
                                    ...
                                    @if ($photoPreview)
                                        <img class="media-preview mb-3" src="{{ $photoPreview }}" alt="Foto cliente">
                                        <div>
                ...
                <div class="modal-footer border-0 px-4 pb-4 pt-0">
                    ...
                    <button type="button" class="btn btn-primary px-4" @if ($Id == 0) wire:click.prevent="store" @else wire:click.prevent="update({{ $Id }})" @endif>
                        {{ $Id ? 'Actualizar testimonio' : 'Guardar testimonio' }}
                    </button>
                </div>
            </div>
        </div>
