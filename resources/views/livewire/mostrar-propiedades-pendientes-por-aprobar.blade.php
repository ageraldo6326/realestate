<div class="property-approval" aria-label="Listado para aprobar propiedades">
    ...
    <div class="approval-summary row">
        ...
        ] as $filter => [$label, $count, $modifier])
            <div class="col-12 col-md-4 mb-3">
                <button type="button" wire:click="setEstado('{{ $filter }}')" class="approval-summary__item text-left w-100 {{ $estado === $filter ? 'is-active ' . $modifier : '' }}" aria-pressed="{{ $estado === $filter ? 'true' : 'false' }}">
                    <span>{{ $label }}</span><strong>{{ number_format($count) }}</strong>
                </button>
            </div>
        @endforeach
    </div>
    ...
    <div class="approval-list" wire:loading.class="is-loading" wire:target="criterio, estado, setEstado">
        @forelse ($propiedades as $propiedad)
            ...
            @endphp
            <article class="approval-card" wire:key="approval-property-{{ $propiedad->id }}" data-approval-card="{{ $propiedad->id }}">
                <img class="approval-card__image" src="{{ $resolveImage($propiedad->foto_portada) }}" alt="Portada de {{ $propiedad->titulo }}" onerror="this.onerror=null;this.src='{{ asset('assets/prop-apto-1.jpg') }}';">
                <div class="approval-card__content">
                    <div class="approval-card__heading">
                        <div><p class="approval-card__reference">{{ $propiedad->referencia ?: 'ID ' . $propiedad->id }}</p><h2>{{ $propiedad->titulo ?: 'Propiedad sin título' }}</h2></div>
                        <span class="approval-badge {{ $approved ? 'approval-badge--approved' : 'approval-badge--pending' }}" data-approval-badge><i class="fas {{ $approved ? 'fa-check-circle' : 'fa-clock' }}" aria-hidden="true"></i><span data-approval-badge-text>{{ $approved ? 'Aprobada' : 'Pendiente' }}</span></span>
                    </div>
                    <dl class="approval-card__metadata">
                        ...
                        <div><dt><i class="fas fa-user" aria-hidden="true"></i> Creada por</dt><dd>{{ $creator ?: 'No disponible' }}</dd></div>
                        <div><dt><i class="fas fa-calendar-alt" aria-hidden="true"></i> Registrada</dt><dd>{{ optional($propiedad->created_at)->format('d/m/Y') ?: 'No disponible' }}</dd></div>
                    </dl>
                </div>
                <div class="approval-card__actions">
                    <div class="approval-control">
                        <div class="approval-control__copy"><span class="approval-control__title">Estado de aprobación</span><span id="approval-status-{{ $propiedad->id }}" class="approval-control__status" data-approval-label>{{ $approved ? 'Aprobada' : 'Pendiente de aprobación' }}</span></div>
                        <label class="approval-switch" for="approval-toggle-{{ $propiedad->id }}"><span class="sr-only">{{ $approved ? 'Retirar aprobación de' : 'Aprobar' }} {{ $propiedad->titulo }}</span><input id="approval-toggle-{{ $propiedad->id }}" type="checkbox" role="switch" aria-describedby="approval-status-{{ $propiedad->id }}" class="js-approval-toggle" data-approval-url="{{ route('admin.propiedades.aprobacion', $propiedad->id) }}" data-approved="{{ $approved ? '1' : '0' }}" {{ $approved ? 'checked' : '' }}><span class="approval-switch__track" aria-hidden="true"></span></label>
                        <span class="approval-spinner" aria-hidden="true"><i class="fas fa-circle-notch fa-spin"></i></span>
                    </div>
                    <div class="approval-card__links"><a href="{{ route('vercualquierpropiedad', $propiedad->id) }}" class="btn btn-outline-secondary">Ver detalle</a><a href="{{ route('editarpendientes', $propiedad->id) }}" class="btn btn-primary">Editar</a></div>
                </div>
            </article>
        @empty
            <div class="approval-empty" role="status"><i class="fas fa-search" aria-hidden="true"></i><h2>No encontramos propiedades</h2><p>Prueba con otro término o cambia el filtro de estado.</p>@if ($criterio !== '' || $estado !== 'todos')<button type="button" wire:click="clearFilters" class="btn btn-outline-primary">Limpiar filtros</button>@endif</div>
        @endforelse
    </div>

    @if ($propiedades->hasPages())<div class="approval-pagination">{{ $propiedades->links('pagination::bootstrap-4') }}</div>@endif
