...
@section('page_title', 'Calendario de tareas')
      ...
      <div class="card-body py-4" style="background: linear-gradient(135deg, #0f172a, #2563eb); color: #ffffff;">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
          ...
          <div class="d-flex flex-wrap gap-2">
            <span class="badge badge-light px-3 py-2">{{ count($eventos) }} eventos</span>
            <span class="badge badge-warning px-3 py-2">{{ $hoyCount }} hoy</span>
            <span class="badge badge-danger px-3 py-2">{{ $vencidasCount }} vencidas</span>
          </div>
        </div>
      </div>
    </div>
              ...
              <div class="border rounded px-3 py-3 mb-3" style="background:#f8fafc;">
                <div class="d-flex align-items-start justify-content-between gap-2">
                  <div>
                    ...
                    <div class="small text-muted">{{ $tarea->cliente ?: 'Sin cliente' }}</div>
                  </div>
                  <span class="badge badge-light border">{{ $tarea->todo_tipo }}</span>
                </div>
                <div class="small text-muted mt-2">{{ \Carbon\Carbon::parse($tarea->fechaLimite)->format('d/m/Y h:i A') }}</div>
              </div>
            @empty
