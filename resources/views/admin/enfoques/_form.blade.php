...
<div class="container-fluid px-3">
    <div class="content-form-shell">
        <section class="content-form-hero">
            <h2 class="h4 mb-2 font-weight-bold">{{ $isEdit ? 'Editar enfoque' : 'Nuevo enfoque' }}</h2>
        ...
        <form action="{{ $action }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text"
                                    ...
                                    value="{{ old('titulo', optional($enfoqueActual)->titulo) }}" maxlength="255"
                                    required>
                            </div>

                            <div class="form-group mb-0">
                                <label for="enfoque-editor" class="content-form-label">Contenido</label>
                                <textarea class="form-control content-form-control @error('enfoque') is-invalid @enderror" id="enfoque-editor"
                                    name="enfoque" rows="10">{{ old('enfoque', optional($enfoqueActual)->enfoque) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-xl-4">
                    ...
                    @if ($currentImage)
                        <div class="content-preview-card mb-4">
                            ...
                            <img src="{{ $currentImage }}" alt="Imagen actual del enfoque">
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="content-submit-row">
                <a href="{{ route('enfoques.index') }}" class="btn btn-outline-secondary px-4">Cancelar</a>
                <button type="submit" class="btn btn-primary px-4 js-submit-button"
                    data-loading-text="Guardando...">{{ $isEdit ? 'Guardar cambios' : 'Guardar enfoque' }}</button>
            </div>
        </form>
    </div>
</div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const target = document.getElementById('enfoque-dropzone-{{ $mode }}');
            ...
            const form = document.getElementById('enfoque-form-{{ $mode }}');
