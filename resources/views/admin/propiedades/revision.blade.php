...
@section('breadcrumb')
    @can('access-admin')
        <li class="breadcrumb-item"><a href="{{ route('poraprobar') }}">Aprobar propiedades</a></li>
    @else
...
@section('page_title', 'Detalle de propiedad')
...
@section('content')
    @php
        ...
        $description = trim(strip_tags((string) $propiedad->descripcion));
    @endphp

    <div class="container-fluid property-review py-4">
        <header class="property-review__hero">
            <div>
                <a href="{{ $returnUrl }}" class="property-review__back"><i class="fas fa-arrow-left" aria-hidden="true"></i> {{ $returnLabel }}</a>
                <p class="property-review__reference">{{ $propiedad->referencia ?: 'ID ' . $propiedad->id }}</p>
                <h1>{{ $propiedad->titulo ?: 'Propiedad sin título' }}</h1>
                <p class="property-review__location"><i class="fas fa-map-marker-alt" aria-hidden="true"></i> {{ $location ?: 'Ubicación no registrada' }}</p>
            </div>
            <div class="property-review__actions">
                <span class="property-review__badge {{ $approved ? 'is-approved' : 'is-pending' }}"><i class="fas {{ $approved ? 'fa-check-circle' : 'fa-clock' }}" aria-hidden="true"></i> {{ $approved ? 'Aprobada' : 'Pendiente de aprobación' }}</span>
                @if ($canManageApproval)
                    <a href="{{ route('editarpendientes', $propiedad->id) }}" class="btn btn-light"><i class="fas fa-pen mr-1" aria-hidden="true"></i> Editar propiedad</a>
                @endif
            </div>
        </header>
        ...
        <div class="row mt-4">
            <div class="col-lg-8 mb-4">
                <div class="card border-0 shadow-sm property-review__card overflow-hidden">
                    <img src="{{ $resolveImage($propiedad->foto_portada) }}" class="property-review__cover" alt="Portada de {{ $propiedad->titulo }}" onerror="this.onerror=null;this.src='{{ asset('assets/prop-apto-1.jpg') }}';">
                    <div class="card-body p-4">
                        <h2>Resumen de la propiedad</h2>
                        <p class="property-review__short-description">{{ $propiedad->descripcion_corta ?: 'No hay resumen registrado.' }}</p>
                        @if ($description !== '')
                            <div class="property-review__description">{!! nl2br(e($description)) !!}</div>
                        @endif
                    </div>
                </div>
