...
class AsignarContacto extends Component
{
    use WithPagination;
    ...
    {
        $user = Auth::user();
        $isAdmin = $user && (int) $user->rol === 1;

        $query = DB::table('clientes')
            ->leftJoin('users as captador', 'clientes.captado_por', '=', 'captador.id')
            ->leftJoin('users as asignado', 'clientes.asignado_a', '=', 'asignado.id')
            ->select('clientes.*', 'captador.name as captador_name', 'asignado.name as asignado_name');

        if (!$isAdmin) {
            $query->where(function ($q) use ($user) {
                $q->where('clientes.captado_por', '=', $user->id)
                    ->orWhere('clientes.asignado_a', '=', $user->id);
            });
        }
        ...
        return view('livewire.asignar-contacto', compact("clientes", "usuarios", "zonas", "tipos_propiedades", "estados_propiedad", "notas", "tareas", "tipos", "estatuses"));
    }
    ...
    public function asignarContacto($id)
    {
        ...
        $this->asignado_a = optional($this->contacto_actual)->asignado_a;
    }
    ...
    public function asignar()
    {
        ...
        $this->dispatchBrowserEvent('close-modal-asignar');
        $this->contacto_actual->asignado_a = $this->asignado_a;
        $this->contacto_actual->save();
    }
}
